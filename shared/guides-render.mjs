...
/* The partner mark, when there is one. It shares the masthead's right
   column with the generated-on line rather than replacing it: the
   document still has to say when it was made and how many countries it
   covers, and those are facts about the document rather than furniture
   a logo may displace.
   The mark's own sizing rules are interpolated from shared/partners.mjs
   so the planner and the guide cannot drift apart on how big a partner
   logo is allowed to be. */
.cover .mast .right{display:flex;flex-direction:column;align-items:flex-end;gap:7px}
${PARTNER_MARK_CSS}
...
export function renderGuideDocument({ bundle, order, lang, strings, today, siteOrigin,
  ...
  <div class="mast">
    ...
    <div class="right">
      ...
      ${partnerMarkHtml(partner, t("doc.partnerBy", "Generated by"), siteOrigin || "")}
    </div>
  </div>
