...
| --- | --- | --- |
| `translations`, namespace `roi` | every string on the planner | **417 keys** |
| `roi_benchmark_translations` | benchmark `label`, `hint`, `citation` | **30 rows × 3 = 90 cells** |
| `roi_phase_translations` | phase `name`, `note` | **7 rows × 2 = 14 cells** |
| `country_translations` | jurisdiction display names | **71 rows** |
| `SUPPORTED_LANGS` (code) | which codes the routers accept | 2 declarations |

The planner's own coverage figure — what `tests/roi-coverage.mjs`
reports — is the first three: **521 cells**. Country names are counted
...
`SUPPORTED_LANGS` exists **twice**, currently identical:

- `shared/deep-dive-render.mjs:26` — exported, imported by site-worker
- `members-worker/src/index.js:94` — its own copy, despite that file
  already importing from the same shared module

Add the code to both, or the public site will offer the language and the
members site will quietly refuse it. Better: delete the members-worker
copy and import the shared one. That is a one-line change and it removes
a drift risk that only ever bites on the day someone adds a language.
