...
function build(){
  ...
  document.getElementById('summary').innerHTML = \`
    ...
    <div class="grid g5">
      ...
      <div class="stat"><div class="n" style="color:\${netAnnual>=0?'#7fd0a8':'#e0907f'}">\${fmt(netAnnual)}</div><div class="l">${tj("res.netAnnual","Net annual")}</div></div>
    ...
    <div class="note" style="margin-top:14px">\${banked
      ...
      : \`<strong>${tj("sum.scopeOnly","Scope: compliance only.")}</strong> \${fmt(l1Banked + l2)} ${tj("sum.scopeOnly2","banks from the integration itself; the remaining")} \${fmt(l1Unbanked)} ${tj("sum.scopeOnly3","needs a change programme you are not running.")}\`} ${tj("sum.bridge","Net annual is the banked figure less")} \${fmt(cPlat)} ${tj("sum.bridge3","of platform fees")}${hlp('cPlat','What this covers')} ${tj("sum.bridge4","and")} \${fmt(cRun)} ${tj("sum.bridge5","of internal run cost; section 4 shows what makes up the banked figure, row by row.")} \${notesLink()}</div>
  ...
  document.getElementById('savingsTable').innerHTML = \`
    ...
    <tr class="tierA" data-row="cycle"><td>${t("row.cycle","Faster cycle time &amp; fewer supplier queries")} <span class="tag intang">intangible</span></td><td>${tj("row.cycle.basis","Top-performing AP spends")} <strong>12.8%</strong> ${tj("row.cycle.basis2","of staff time on supplier inquiries against")} <strong>24.0%</strong> \${ev('ardentInq','Ardent Partners, 2025 data')} &mdash; ${tj("row.cycle.basis3","an association with high-performing AP, not a measured effect of e-invoicing, so")} <strong>${tj("row.cycle.basis4","not monetised")}</strong>. \${notesLink()}</td>\${dash}</tr>

    <tr class="tierA" data-row="paper"><td>Paper, print, postage, storage <span class="tag tang">tangible</span></td><td>Paper AUD 30.87 vs e-invoice AUD 9.18 \${ev('ato','ATO / Deloitte')}; your own spend is the better input</td>\${dash}</tr>

    <tr class="tierC" data-row="vat"><td>VAT leakage / gap recovery <span class="tag intang">intangible</span></td><td>Often quoted, <strong>not defensible</strong> \${ev('vatgap','why not')} &mdash; excluded from this model entirely</td>\${dash}</tr>
