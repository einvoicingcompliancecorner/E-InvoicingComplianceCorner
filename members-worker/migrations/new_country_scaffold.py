...
Spec shape:
{
  "name": "Qatar", "code": "QA", "region": "Middle East / Africa", "slug": "qatar",
  "names": { "es": "Catar", "de": "Katar", "fr": "Qatar" },
  "roi_complexity": "complex",   // required: 'complex' | 'simple' | 'none'
                                  // -- see 510_roi_complexity_column.sql. Required
                                  // (not defaulted) for the same reason mandate_scope
                                  // is: the column's NOT NULL DEFAULT is 'none', which
                                  // prices the country at zero integrations in the ROI
                                  // planner AND exempts it from the weekly complexity
                                  // review, which only inspects countries rated
                                  // 'simple'. Omission is silent in both directions.
  "eu_member": false,             // optional, default false: does ViDA bind this
                                  // country? All 27 member states already exist, so
                                  // false is almost certainly right.
  "milestones": [
    {
      "id": "qa-b2b-phase1",
      "date": "2027-01-01",
...
REGIONS = {"Europe", "Middle East / Africa", "Asia-Pacific", "Americas"}
LANGS = ("es", "de", "fr")
...
def validate(spec):
    ...
    if spec.get("roi_complexity") not in ("complex", "simple", "none"):
        fail("roi_complexity must be 'complex', 'simple' or 'none' (see "
             "510_roi_complexity_column.sql). It is REQUIRED rather than defaulted: "
             "the column's NOT NULL DEFAULT is 'none', which contributes zero "
             "integrations to the ROI planner's one-off cost and is invisible to the "
             "weekly complexity review, which only scans countries rated 'simple'. "
             "The rule is whether the tax authority is a party to the transaction.")
    if not isinstance(spec.get("eu_member", False), bool):
        fail("eu_member must be true or false")
    if spec.get("eu_member"):
        print("  note: eu_member is true. All 27 member states already exist, and a "
              "standing invariant asserts exactly 27 -- check this is really an "
              "EU-27 state before applying.")
...
def gen_country_sql(spec):
    lines = [
        ...
        f"INSERT OR IGNORE INTO countries (code, name_en, region, slug, in_picker, roi_complexity, eu_member) "
        f"VALUES ('{esc(spec['code'])}', '{esc(spec['name'])}', '{esc(spec['region'])}', "
        f"'{esc(spec['slug'])}', 1, '{esc(spec['roi_complexity'])}', {1 if spec.get('eu_member') else 0});",
        "",
    ...
    lines += [
        ...
        f"-- ASSERT: SELECT roi_complexity FROM countries WHERE code = '{code}' = '{esc(spec['roi_complexity'])}'",
        f"-- ASSERT: SELECT eu_member FROM countries WHERE code = '{code}' = {1 if spec.get('eu_member') else 0}",
...
def main():
    ...
    print("  2. HEADLINE FACTS: country_headline_facts + its notes in four")
    print("     languages + fact_history rows. Nothing renders without them and")
    print("     the deep dive loses its strip silently. Grade any new citation")
    print("     host in source_hosts at the same time or the replay refuses.")
    print("  3. Deep-dive content migrations (DEEP-DIVE-MIGRATION-CHECKLIST.md)")
    print("  4. tracking_sources, or the country is watched by nothing")
    print("  5. countries.js + shared/deep-dive-render.mjs slug map + i18n regen")
    print("  6. The jurisdiction-count sweep: npm run count:fix")
    print("  7. python3 apply_migrations.py --remote   (validates, applies, records)")
    print("  8. Check /map after deploy: browser console for \"no map position for")
    ...
    print("     Check the country's decoded extent against REGION_BOUNDS too: a")
    print("     country outside its own region box is projected off screen with")
